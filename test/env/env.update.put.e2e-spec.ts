...
describe.each(testTypes())(`${EnvController.name} (e2e) (%s)`, (type) => {
  describe('/users/:userId/envs/:envId (PUT)', () => {
    ...
    it(`should return ${HttpStatus.UNAUTHORIZED} if not authorized`, async () => {
      ...
      await request(app.getHttpServer())
        .put(`/users/${faker.string.uuid()}/envs/${faker.string.uuid()}`)
        .send(createDto)
    ...
    it(`should return ${HttpStatus.NOT_FOUND} if user not found`, async () => {
      ...
      await request(app.getHttpServer())
        .put(`/users/${user.id}/envs/${env.body.id}`)
        .set("Authorization", `Bearer ${loginResponse.body.accessToken}`)
        .send(dto)
        .expect(HttpStatus.NOT_FOUND);
    });

    it(`should return ${HttpStatus.FORBIDDEN} if user tries to update env of other user`, async () => {
      ...
      await request(app.getHttpServer())
        .put(`/users/${otherUser.id}/envs/${env.body.id}`)
        .set("Authorization", `Bearer ${loginResponse.body.accessToken}`)
        .send(dto)
    ...
    it(`should return ${HttpStatus.NOT_FOUND} if env does not exist`, async () => {
      ...
      await request(app.getHttpServer())
        .put(`/users/${user.id}/envs/${faker.string.uuid()}`)
        .set("Authorization", `Bearer ${loginResponse.body.accessToken}`)
        .send({ name: "Test" })
        .expect(HttpStatus.NOT_FOUND);
    });

    const mockInvalidDtos: UpdateEnvDto[] = [
    ...
    it.each(mockRoles)("should update env for %s if done by admin", async (role) => {
      ...
      expect(env.body.id)
        .toEqual(createResponse.body.id);
    })

    const mockValidDtos: CreateEnvDto[] = [
    ...
    it.each(mockValidDtos)("update env for the user", async (dto) => {
