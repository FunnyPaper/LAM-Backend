...
describe.each(testTypes())(`${EnvController.name} (e2e) (%s)`, (type) => {
  describe('/users/:userId/envs/:envId (DELETE)', () => {
    ...
    it(`should return ${HttpStatus.UNAUTHORIZED} if not authorized`, async () => {
      await request(app.getHttpServer())
        .delete(`/users/${faker.string.uuid()}/envs/${faker.string.uuid()}`)
        .send()
    ...
    it(`should return ${HttpStatus.NOT_FOUND} if user not found`, async () => {
      ...
      await request(app.getHttpServer())
        .delete(`/users/${user.id}/envs/${env.body.id}`)
        .set("Authorization", `Bearer ${loginResponse.body.accessToken}`)
        .send()
        .expect(HttpStatus.NOT_FOUND);
    });

    it(`should return ${HttpStatus.FORBIDDEN} if user tries to delete env of other user`, async () => {
      ...
      await request(app.getHttpServer())
        .delete(`/users/${otherUser.id}/envs/${env.body.id}`)
        .set("Authorization", `Bearer ${loginResponse.body.accessToken}`)
        .send()
    ...
    it(`should return ${HttpStatus.NOT_FOUND} if env does not exist`, async () => {
      ...
      await request(app.getHttpServer())
        .delete(`/users/${user.id}/envs/${faker.string.uuid()}`)
        .set("Authorization", `Bearer ${loginResponse.body.accessToken}`)
        .send()
        .expect(HttpStatus.NOT_FOUND);
    });
    ...
    it.each(mockRoles)("should delete env for %s if done by admin", async (role) => {
      ...
      await expect(getEnvs(app, otherUser.id))
        .resolves
        .toContainEqual(expect.objectContaining({ id: env.body.id }));
    ...
    it("should delete env for the user", async () => {
