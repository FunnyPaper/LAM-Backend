  ...
  describe('/users/:userId/envs (GET)', () => {
    ...
    it(`should return ${HttpStatus.UNAUTHORIZED} if not authorized`, async () => {
      await request(app.getHttpServer())
        .get(`/users/${faker.string.uuid()}/envs`)
        .send()
    ...
    it(`should return ${HttpStatus.NOT_FOUND} if user not found`, async () => {
      ...
      await request(app.getHttpServer())
        .get(`/users/${user.id}/envs`)
        .set("Authorization", `Bearer ${loginResponse.body.accessToken}`)
        .send()
        .expect(HttpStatus.NOT_FOUND);
    });

    it(`should return ${HttpStatus.FORBIDDEN} if user tries to query envs of other user`, async () => {
    ...
    it.each(mockRoles)("should query envs for %s if done by admin", async (role) => {
      ...
      expect(queryResponse.body)
        .toContainEqual(expect.objectContaining(env.body));
    })

    it("should query envs for the user", async () => {
